...
public class ItemView : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    public static ItemView Create(ISessionSettings settings, InventorySlot model, int ownerId)
    {
        if (_prefab == null)
        {
        ...
        createdObject.ItemActionButton.image.sprite = ownerId == settings.PlayerId ? buttonStates[1] : buttonStates[0]; //TODO: Change!
